(function () {
    let form = document.querySelector('#js-category-form')
    ...
    form.addEventListener('submit', function (e) {
        ...
        post(url, data, function (xhr) {
            ...
            sucessAlert('La catégories à été ajouté avec success')
...
(function () {
    ...
    table.addEventListener('click', function (e) {
            ...
            warningAalert('Supprimer', 'Etes vous sur ?').then(function (result) {
                if (result.value) {
                    get(url, function (xhr) {
                        sucessAlert('La catégories a été supprimée')
                        $('#js-table').load(document.URL + '#body  tr')
                    }, function (xhr) {
                        errorAlert()
                    })
        ...
        if (e.target.classList.contains('js-unlock')) {
            let url = e.target.getAttribute('data-url')
            warningAalert('Bloquer', 'La Catégorie sera bloquer').then(function (result) {
                if (result.value) {
                    get(url, function (xhr) {
                        sucessAlert('La catégories a été supprimée')
                        $('#js-table').load(document.URL + '#body  tr')
...
(function () {
    let forms = document.querySelectorAll('.js-category-form')
    forms.forEach(form => {
        let formBtn = form.querySelector('button')
        form.addEventListener('submit', function (e) {
            e.preventDefault()
            ...
            let url = form.getAttribute('action')
            post(url, data, function (xhr) {
                sucessAlert('La catégories à été mis à jour avec success')
                formBtn.disabled = false
