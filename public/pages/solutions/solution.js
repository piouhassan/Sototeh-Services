(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        post(url, data, function (xhr) {
            ...
            sucessAlert('La solution à été ajouté avec success')
...
(function () {
    ...
    table.addEventListener('click', function (e) {
            ...
            warningAalert('Supprimer', 'Etes vous sur ?').then(function (result) {
                if (result.value) {
                    get(url, function (xhr) {
                        sucessAlert('La solution a été supprimée')
                        $('#js-table').load(document.URL + '#body  tr')
                    }, function (xhr) {
                        errorAlert()
                    })
        ...
        if (e.target.classList.contains('js-unlock')) {
            let url = e.target.getAttribute('data-url')
            warningAalert('Bloquer', 'La solution sera bloquée').then(function (result) {
                if (result.value) {
                    get(url, function (xhr) {
...
(function () {
    ...
    forms.forEach(form => {
        ...
        form.addEventListener('submit', function (e) {
            ...
            let url = form.getAttribute('action')
            post(url, data, function (xhr) {
                sucessAlert('La catégories a été mis à jour avec success')
                formBtn.disabled = false
