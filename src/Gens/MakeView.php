...
private $path;
    ...
    private function setPath($dirname)
    {
       ...
       $path = dirname($_SERVER['DOCUMENT_ROOT']).'/resources/views/backend/'.$dirname.'/';
    ...
    {
        if (MakeFile::make($this->setPath($filename), $filename . '.twig')) {
            return true;
        } else {
            return false;
        }
        ...
        public function write($filename)
    {
        ...
        $text =
            "{% extends 'backend/layout/main.twig' %}
...
{% endblock %}";
        ...
        if ($this->make($filename)){
            $handle = fopen($this->setPath($filename).$filename.'.twig', 'a+');
            if  ($handle !== false) {
                fwrite($handle, $text);
                fclose($handle);
            }
